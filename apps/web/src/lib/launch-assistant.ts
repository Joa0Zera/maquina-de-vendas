...
export async function getSmartNextAction(productId: string, organizationId: string): Promise<SmartNextAction | null> {
...
export async function getProjectScore(productId: string, organizationId: string): Promise<number> {
  const [product, offer, research, copy, organic, intelligence] = await Promise.all([
    db
      .select()
      .from(products)
...
export async function getProjectStatus(score: number): Promise<string> {
